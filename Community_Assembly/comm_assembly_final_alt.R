###This alternative script runs all analyses using the fungal phylogenetic tree derived from UNITE taxonomy, i.e., 'fungtree2.tre'
###These results are nearly identical to the results from the ITS sequence alignment tree and are presented in the supplmentary
###information


library(phyloseq)

fung_otus <- "C:/Users/ernie/OneDrive/Desktop/phylogenetic analyses/FD-ITS-table-with-taxonomy.biom"
...
tax <- data.frame(tax_table(OTU2))

write.csv(tax,"C:\\Users\\ernie\\OneDrive\\Desktop\\phylogenetic analyses\\fungtax.csv", row.names = TRUE)
...
fungtree <-  read.tree("C:/Users/ernie/OneDrive/Desktop/phylogenetic analyses/fungtree2.tre")
...
cor2 <- mantel.correlog(soil_dist2.5, phydist2.5,n.class = 50, mult="bonferroni",nperm=1)
...
fung_assem2[fung_assem2 =="Heterogeneous.Selection"] <- "Variable Selection"

fung_assem2$variable <- factor(fung_assem2$variable, levels = c("Homogenizing Dispersal", "Dispersal Limitation", "Drift", "Variable Selection","Homogeneous Selection"))
...
ord6 <- capscale(as.dist(fung_bnti1)~vars$TDN+vars$DOC.TDN+vars$TN+Condition(vars$blen+vars$carya+vars$ltul+vars$pstr))

anova(ord6)

anova.cca(ord6, by="term")

ord7 <- capscale(as.dist(fung_bnti1)~vars$blen+vars$carya+vars$ltul+vars$pstr+Condition(vars$TDN+vars$DOC.TDN+vars$TN))
