...
t0 <- read.csv("C:/Users/ernie/Desktop/Coweeta_drought/Drought_t0.csv")
...
water <- read.csv("C:/Users/ernie/Desktop/Coweeta_drought/Drought_water.csv")
...
glm1 <- glmer(GVM~ LandUse*Treatment + (1|Plot), data=water, family=Gamma(link=log))
Anova(glm1)

#Respiration responses

resp2 <- read.csv("C:/Users/ernie/Desktop/Coweeta_drought/Drought_resp2.csv")
...
resp <- read.csv("C:/Users/ernie/Desktop/Coweeta_drought/Drought_resp.csv")
...
jpeg(filename="Figure2.jpeg", bg="transparent", res=600, units = "in", height=4.5, width=7) 
...
line_plotCmin <- ggplot(resp2.2, aes(x=resp2.2$Day, y=resp2.2$Cmin.mean, group=resp2.2$Treatment)) + 
  ...
  annotate('text', 70, 3.5, label="Disturbance: italic(P) == 0.770", size = 4, parse=TRUE)+
  annotate('text', 70, 3.25, label="Disturbance %*% Drought: italic(P) == 0.509", size=4, parse=TRUE)+
...
glm3 <- glmer(Cmin~LandUse*Drought*Day+(1|Plot), data=resp, family=Gamma(link=log),nAGQ=0,glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1000000)))
Anova(glm3, type="III")
...
din <- read.csv("C:/Users/ernie/Desktop/Coweeta_drought/drought_chem.csv")

din$Day <- as.factor(din$Day)

#Relationships of AOB with NO3 on day 84 (fig. S7)
...
glmfb <- glmer(ITS.16S ~ LandUse*Drought*Day + (1|Plot), data=din, family=Gamma(link=log),glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
Anova(glmfb)
...
bar_plotFB<- ggplot(FB2, aes(x=FB2$Day, y=FB2$ITS.16S.mean, fill=FB2$Treatment)) + 
  ...
  annotate('text', 3.5, .18, label="Drought: ~italic(P) == 0.021", size=5, parse=TRUE)+
  annotate('text', 3.5, .17, label="Disturbance: italic(P) == 0.067", size = 5, parse=TRUE)+
