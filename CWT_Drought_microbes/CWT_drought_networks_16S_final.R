...
drought_asvs <- "C:/Users/ernie/Desktop/Coweeta_drought/16S-table-with-taxonomy.biom"
...
legend('center',legend=unique(V(g1.11)$Phylum),cex=1.5,pt.cex=2.5,pch=21,pt.bg=unique(V(g1.11)$phylacolor))
dev.off()
...
bar_plot_degree<- ggplot(deg, aes(x=time, y=degree.mean, fill=Treatment)) + 
  ...
  annotate('text', 1.25, .0061, label="Disturbance: italic(P) < 0.001", size = 4,fontface="bold", parse=TRUE)+
  annotate('text', 1.25, .0057, label="Disturbance %*% Drought: italic(P)< 0.001", size=4,fontface="bold", parse=TRUE)+
  theme_classic() +
...
bar_plot_between<- ggplot(bet, aes(x=time, y=between.mean, fill=Treatment)) + 
  ...
  annotate('text', 1.25, .0006, label="Disturbance: italic(P) < 0.001", size = 4, parse=TRUE)+
  annotate('text', 1.25, .00055, label="Disturbance %*% Drought: italic(P) == 0.612", size=4, parse=TRUE)+
  theme_classic() +
  scale_fill_brewer(palette="Set1") +
  theme(axis.title=element_text(size=20)) +
  theme(text=element_text(size=20)) +
