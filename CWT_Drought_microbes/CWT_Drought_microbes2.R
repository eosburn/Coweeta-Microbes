...
line_plot_bac_alpha <- ggplot(bac_shan2, aes(x=bac_shan2$Day, y=bac_shan2$Shannon.mean, group=bac_shan2$Treatment)) + 
  ...
  annotate('text',5.3, 5.33, label="Disturbance: italic(P) < 0.001", size = 3, parse=TRUE)+
  annotate('text', 5.3, 5.26, label="Disturbance %*% Drought: italic(P) == 0.664", size=3, parse=TRUE)+
  labs(list(x ="")) +
  theme_classic() +
...
line_plot_fung_alpha <- ggplot(fung_shan2, aes(x=fung_shan2$Day, y=fung_shan2$Shannon.mean, group=fung_shan2$Treatment)) + 
  ...
  annotate('text', 4.8, 1.88, label="Disturbance: italic(P) == 0.67", size = 3, parse=TRUE)+
  annotate('text', 4.8, 1.76, label="Disturbance %*% Drought: italic(P) == 0.684", size=3, parse=TRUE)+
  labs(list(x ="")) +
  theme_classic() +
  scale_color_brewer(palette="Set1") +
  geom_vline(xintercept=2.5, linetype="dashed", color = "black",size=1) +
...
line_plot_bac_pairs <- ggplot(bac_pairs2, aes(x=bac_pairs2$Day, y=bac_pairs2$Distance.mean, group=bac_pairs2$LandUse)) + 
  ...
  annotate('text', 4.5, .43, label="Disturbance: italic(P) == 0.004", size = 6, parse=TRUE)+
...
line_plot_rk <- ggplot(rk1.1, aes(x=rk1.1$Day, y=rk1.1$rk.mean, group=rk1.1$Treatment)) + 
  ...
  annotate('text', 5.4, 2.85, label="Disturbance: italic(P) == 0.014", size = 3, parse=TRUE)+
  annotate('text', 5.4, 2.7, label="Disturbance %*% Drought: italic(P) == 0.278", size=3, parse=TRUE)+
  labs(list(x ="")) +
  theme_classic() +
  geom_vline(xintercept=2.5, linetype="dashed", color = "black",size=1) +
  annotate("rect", xmin=-Inf, xmax=2.5, ymin = -Inf, ymax = Inf, alpha = .3, fill = "#999999") +
