...
line_plot_bac_alpha <- ggplot(bac_shan2, aes(x=bac_shan2$Day, y=bac_shan2$Shannon.mean, group=bac_shan2$Treatment)) + 
  ...
  annotate('text', 5.3, 5.4, label="Drought: ~italic(P) < 0.001", size=3, parse=TRUE)+
  annotate('text',5.3, 5.33, label="Land~Use: italic(P) < 0.001", size = 3, parse=TRUE)+
  annotate('text', 5.3, 5.26, label="Land~Use %*% Drought: italic(P) == 0.664", size=3, parse=TRUE)+
  labs(list(x ="")) +
  theme_classic() +
  geom_vline(xintercept=2.5, linetype="dashed", color = "black",size=1) +
  annotate("rect", xmin=-Inf, xmax=2.5, ymin = -Inf, ymax = Inf, alpha = .3, fill = "#999999") +
  scale_color_brewer(palette="Set1") +
  theme(axis.title=element_text(size=15)) +
  theme(text=element_text(size=20)) +
  theme(legend.position=c(.17,.2)) +
...
addSmallLegend <- function(myPlot, pointSize = 1, textSize = 8, spaceLegend = 0.05) {
  myPlot +
    ...
    theme(legend.title = element_blank(), 
          ...
          legend.margin=margin(-7,5,5,5))
}

line_plot_bac_alpha<- addSmallLegend(line_plot_bac_alpha)
...
line_plot_fung_alpha <- ggplot(fung_shan2, aes(x=fung_shan2$Day, y=fung_shan2$Shannon.mean, group=fung_shan2$Treatment)) + 
  ...
  annotate('text', 4.8, 2.0, label="Drought: ~italic(P) == 0.024", size=3, parse=TRUE)+
  annotate('text', 4.8, 1.88, label="Land~Use: italic(P) == 0.67", size = 3, parse=TRUE)+
  annotate('text', 4.8, 1.76, label="Land~Use %*% Drought: italic(P) == 0.684", size=3, parse=TRUE)+
  labs(list(x ="")) +
  theme_classic() +
  scale_color_brewer(palette="Set1") +
  geom_vline(xintercept=2.5, linetype="dashed", color = "black",size=1) +
...
resist1$Treatment<- factor(resist1$Treatment, levels = c("Reference-Drought", "Disturbed-Drought"))

box_plotResist_alpha<- ggplot(resist1, aes(x=resist1$Taxon, y=resist1$resist, fill=resist1$Treatment)) + 
  ...
  stat_summary(inherit.aes=FALSE,aes(x=resist1$Taxon, y=resist1$resist, group=resist1$Treatment),fun.y=mean,geom="point",shape=23,size=4,color="black",fill="white", position=position_dodge(.75))+
...
resil1$Treatment<- factor(resil1$Treatment, levels = c("Reference-Drought", "Disturbed-Drought"))


box_plotResil_alpha<- ggplot(resil1, aes(x=resist1$Taxon, y=resil1$resil, fill=resil1$Treatment)) + 
  stat_boxplot(geom = "errorbar", size=.5,width = 0.5, position=position_dodge(.75)) +
  geom_boxplot(inherit.aes = TRUE,size=.5, outlier.size=1) +
  stat_summary(inherit.aes=FALSE,aes(x=resil1$Taxon, y=resil1$resil, group=resil1$Treatment),fun.y=mean,geom="point",shape=23,size=4,color="black",fill="white", position=position_dodge(.75))+
  ...
  theme(axis.line.y=element_line(colour="black", size=1)) + 
  theme(legend.title=element_blank()) +
  scale_y_continuous(expression("Resilience")) +
  theme(legend.position="none") +
  theme(axis.title.x=element_blank())
...
bac_dist <- read.csv("C:/Users/ernie/OneDrive/Desktop/Coweeta_drought/16S-distances.csv")
...
line_plot_bac_beta <- ggplot(bac_beta2, aes(x=bac_beta2$Day, y=bac_beta2$Distance.mean, group=bac_beta2$Treatment)) + 
  ...
  annotate('text', 1.5, .66, label="Land~Use: italic(P) < 0.001", size = 5, parse=TRUE)+
  annotate('text', 1.5, .64, label="Land~Use %*% Drought: italic(P) == 0.722", size=5, parse=TRUE)+
  labs(list(x ="")) +
  theme_classic() +
  scale_color_brewer(palette="Set1") +
  theme(axis.title=element_text(size=20)) +
...
bac_pairs <- read.csv("C:/Users/ernie/OneDrive/Desktop/Coweeta_drought/16S-pairs.csv")
...
line_plot_bac_pairs <- ggplot(bac_pairs2, aes(x=bac_pairs2$Day, y=bac_pairs2$Distance.mean, group=bac_pairs2$LandUse)) + 
  ...
  annotate('text', 4.5, .43, label="Land~Use: italic(P) == 0.004", size = 6, parse=TRUE)+
  annotate('text',4.5, .4, label="Day: italic(P) < 0.001", size = 6, parse=TRUE)+
  labs(list(x ="")) +
  theme_classic() +
  geom_vline(xintercept=2.5, linetype="dashed", color = "black",size=1) +
  annotate("rect", xmin=-Inf, xmax=2.5, ymin = -Inf, ymax = Inf, alpha = .3, fill = "#999999") +
  ...
  theme(axis.line.y=element_line(colour="black", size=1)) + 
  theme(legend.title=element_blank(),
        legend.margin=margin(c(-14,0,0,0))) +
...
ord_2 <- ggplot(multi_ord6, aes(x=MDS1.mean, y=MDS2.mean,color=Treatment,shape=Day)) + 
  ...
  annotate("text", .1, -.14, label="Land~Use: ~italic(P) == 0.001", parse=TRUE, size=4.5, fontface="bold")+
  annotate("text", -.15, .11, label="'2D Stress' == 0.20", parse=TRUE, size=4.5, fontface="bold")+
  annotate("text", .1, -.12, label="Drought: ~italic(P) == 0.09", parse=TRUE, size=4.5, fontface="bold")+
  annotate("text", .1, -.16, label="`Land Use x Drought`: ~italic(P) == 0.851", parse=TRUE, size=4.5, fontface="bold")+
  scale_color_brewer(palette="Set1") +
  scale_shape_manual(values = c(15, 16, 17,18,7,4)) +
  xlab("NMDS 1") +
  ylab("NMDS 2") +
...
fung_dist <- read.csv("C:/Users/ernie/OneDrive/Desktop/Coweeta_drought/ITS-distances.csv")
...
line_plot_fung_beta <- ggplot(fung_beta2, aes(x=fung_beta2$Day, y=fung_beta2$Distance.mean, group=fung_beta2$Treatment)) + 
  ...
  annotate('text', 2, .87, label="Land~Use: italic(P) == 0.284", size = 5, parse=TRUE)+
  annotate('text', 2, .85, label="Land~Use %*% Drought: italic(P) == 0.227", size=5, parse=TRUE)+
  labs(list(x ="")) +
  theme_classic() +
  scale_color_brewer(palette="Set1") +
  theme(axis.title=element_text(size=20)) +
...
fung_pairs <- read.csv("C:/Users/ernie/OneDrive/Desktop/Coweeta_drought/ITS-pairs.csv")
...
line_plot_fung_pairs <- ggplot(fung_pairs2, aes(x=fung_pairs2$Day, y=fung_pairs2$Distance.mean, group=fung_pairs2$LandUse)) + 
  ...
  annotate('text', 4.5, .54, label="Land~Use: italic(P) == 0.598", size = 6, parse=TRUE)+
...
box_plotResist_beta<- ggplot(resist2, aes(x=resist2$Taxon, y=resist2$resist, fill=resist2$LandUse)) + 
  stat_boxplot(geom = "errorbar", size=.5,width = 0.5, position=position_dodge(.75)) +
  geom_boxplot(inherit.aes = TRUE,size=.5, outlier.size=.25) +
  stat_summary(inherit.aes=FALSE,aes(x=resist2$Taxon, y=resist2$resist, group=resist2$LandUse),fun.y=mean,geom="point",shape=23,size=4,color="black",fill="white", position=position_dodge(.75))+
...
box_plotResil_beta<- ggplot(resil2, aes(x=resil2$Taxon, y=resil2$resil, fill=resil2$LandUse)) + 
  stat_boxplot(geom = "errorbar", size=.5,width = 0.5, position=position_dodge(.75)) +
  geom_boxplot(inherit.aes = TRUE,size=.5, outlier.size=.25) +
  stat_summary(inherit.aes=FALSE,aes(x=resil2$Taxon, y=resil2$resil, group=resil2$LandUse),fun.y=mean,geom="point",shape=23,size=4,color="black",fill="white", position=position_dodge(.75))+
...
jpeg(filename="drought_rk.jpeg", bg="transparent", res=600, units = "in", height=4.5, width=6) 
...
line_plot_rk <- ggplot(rk1.1, aes(x=rk1.1$Day, y=rk1.1$rk.mean, group=rk1.1$Treatment)) + 
  ...
  annotate('text', 5.4, 3, label="Drought: ~italic(P) < 0.001", size=3, parse=TRUE)+
  annotate('text', 5.4, 2.85, label="Land~Use: italic(P) == 0.014", size = 3, parse=TRUE)+
  annotate('text', 5.4, 2.7, label="Land~Use %*% Drought: italic(P) == 0.278", size=3, parse=TRUE)+
  labs(list(x ="")) +
  theme_classic() +
  geom_vline(xintercept=2.5, linetype="dashed", color = "black",size=1) +
  annotate("rect", xmin=-Inf, xmax=2.5, ymin = -Inf, ymax = Inf, alpha = .3, fill = "#999999") +
...
jpeg(filename="line_plot1.jpeg", bg="transparent", res=600, units = "in", height=6.67, width=7.5)
...
jpeg(filename="line_plot2.jpeg", bg="transparent", res=600, units = "in", height=6.67, width=7.5)
...
jpeg(filename="legends1.jpeg", bg="transparent", res=600, units = "in", height=6.67, width=4)
