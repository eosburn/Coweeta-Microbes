...
Bac_otus <- "C:/Users/ernie/Desktop/Coweeta_drought/16S-table-with-taxonomy.biom"

Fung_otus <- "C:/Users/ernie/Desktop/Coweeta_drought/ITS-table-with-taxonomy.biom"
...
meta <- "C:/Users/ernie/Desktop/QIIME/16S_metadata_CWT.txt"
...
chem <- read.csv("C:/Users/ernie/Desktop/Coweeta_drought/drought_chem.csv")

chem2 <- chem[-16,]

chem2<- chem2[order(chem2$Sample),]

chem2$Bac_Shannon <- bac_alpha$Shannon

library(corrplot)
library(RColorBrewer)
scalebluered <- colorRampPalette(brewer.pal(8, "RdBu"))(8)

Corr1 <- cor(chem2[,c(27,11,12,14,15,17)],use = "complete.obs", method="pearson")

res1<- cor.mtest(chem2[,c(27,11,12,14,15,17)])

jpeg(filename="bac_correlogram.jpeg", bg="transparent", res=500, units = "in", height=3, width=8)

corrplot(Corr1[1,1:6, drop=FALSE], method="color", col=scalebluered, diag=FALSE, addCoef.col = "black",  type="upper",  tl.col="black", tl.srt=45,sig.level=0.05,p.mat = res1$p, insig = "blank")
...
chem3<- chem3[order(chem3$Sample),]

chem3$Fung_Shannon <- fung_alpha$Shannon

library(corrplot)
library(RColorBrewer)
scalebluered <- colorRampPalette(brewer.pal(8, "RdBu"))(8)

Corr2 <- cor(chem3[,c(27,11,12,14,15,17)],use = "complete.obs", method="pearson")

res2<- cor.mtest(chem3[,c(27,11,12,14,15,17)])

jpeg(filename="fung_correlogram.jpeg", bg="transparent", res=500, units = "in", height=3, width=8)

corrplot(Corr2[1,1:6, drop=FALSE], method="color", col=scalebluered, diag=FALSE, addCoef.col = "black",  type="upper",  tl.col="black", tl.srt=45,sig.level=0.05,p.mat = res2$p, insig = "blank")
...
chem_dist <- vegdist(chem2[,c(11,12,14,15,17)], method="euclidean")

mantel(b1,chem_dist, method="pearson")

mantel(b1,b2, method="pearson")
...
chem_dist <- vegdist(chem3[,c(11,12,14,15,17)], method="euclidean")

mantel(b2,chem_dist, method="pearson")
...
guilds <- read.csv("C:/Users/ernie/Desktop/Coweeta_drought/ITS-asvs.guilds.final.csv")

library(plyr)
guilds <- ddply(guilds,"Guild",numcolwise(sum))

library(funrar)
guilds.matrix <- data.matrix(guilds[,c(2:144)])

guilds.rel <- data.frame(make_relative(guilds.matrix))

guilds.rel$Guild <- guilds$Guild

guilds <- guilds.rel

guilds2 <- setNames(data.frame(t(guilds[,-144])), guilds[,144])

guilds3 <- cbind(guilds2, c2)

library(tidyr)
guilds4 <- gather(guilds3, Guild, Abundance, `Animal Pathogen`:`Wood Saprotroph`, factor_key=TRUE)

guilds4 <- guilds4[,-c(1:4)]

guilds5 <- aggregate(Abundance~id+Guild+Treatment+LandUse+Plot+Drought+Day, guilds4, FUN=sum)

guilds5 <- guilds5[-c(6877,4263),]

guilds6 <- cast(guilds5, id+Treatment+LandUse+Plot+Drought+Day ~ Guild, value="Abundance")

guilds6$Day <- as.factor(guilds6$Day)

glm40 <- glmer(`Animal Pathogen`+.00001~ Drought*LandUse*Day + (1|Plot),family=Gamma(link=log), glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),data=guilds6)
qqPlot(resid(glm40))
Anova(glm40)

glm41 <- glmer(Ectomycorrhizal+.0001~ Drought*LandUse*Day + (1|Plot),family=Gamma(link=log), glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),data=guilds6)
qqPlot(resid(glm41))
Anova(glm41)


glm42 <- glmer(Endophyte+.0001~ Drought*LandUse*Day + (1|Plot),family=Gamma(link=log), glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),data=guilds6)
qqPlot(resid(glm42))
Anova(glm42)

glm43 <- glmer(`Ericoid Mycorrhizal`+.0001~ Drought*LandUse*Day + (1|Plot),family=Gamma(link=log), glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),data=guilds6)
qqPlot(resid(glm43))
Anova(glm43)

glm44 <- glmer(`Plant Pathogen`+.0001~ Drought*LandUse*Day + (1|Plot),family=Gamma(link=log), glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),data=guilds6)
qqPlot(resid(glm44))
Anova(glm44)

glm45 <- glmer(`Soil Saprotroph`+.0001~ Drought*LandUse*Day + (1|Plot),family=Gamma(link=log), glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),data=guilds6)
qqPlot(resid(glm45))
Anova(glm45)

glm46 <- glmer(`Undefined Saprotroph`+.0001~ Drought*LandUse*Day + (1|Plot),family=Gamma(link=log), glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),data=guilds6)
qqPlot(resid(glm46))
Anova(glm46)

glm48 <- glmer(`Fungal Parasite`+.0001~ Drought*LandUse*Day + (1|Plot),family=Gamma(link=log), glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),data=guilds6)
qqPlot(resid(glm48))
Anova(glm48)

glm49 <- glmer(`Plant Saprotroph`+.0001~ Drought*LandUse*Day + (1|Plot),family=Gamma(link=log), glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),data=guilds6)
qqPlot(resid(glm49))
Anova(glm49)

glm50 <- glmer(`Wood Saprotroph`+.0001~ Drought*LandUse*Day + (1|Plot),family=Gamma(link=log), glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),data=guilds6)
qqPlot(resid(glm50))
Anova(glm50)

#Funguild line plots

endo1 <- guilds5 [guilds5 $Guild == 'Endophyte',]

wood1 <- guilds5[guilds5 $Guild == 'Wood Saprotroph',]

ericoid1 <- guilds5[guilds5 $Guild == 'Ericoid Mycorrhizal',]

guilds_ag <- aggregate(Abundance~ Treatment + Drought + Day + LandUse + Guild, data=guilds5, FUN =function(x) c(mean=mean(x), sd=sd(x),n=length(x)))

path <- guilds_ag [guilds_ag $Guild == 'Animal Pathogen',]
ecto <- guilds_ag [guilds_ag $Guild == 'Ectomycorrhizal',]
eric <- guilds_ag [guilds_ag $Guild == 'Ericoid Mycorrhizal',]
fung <- guilds_ag [guilds_ag $Guild == 'Fungal Parasite',]
plant <- guilds_ag [guilds_ag $Guild == 'Plant Saprotroph',]
endo <- guilds_ag [guilds_ag $Guild == 'Endophyte',]
und <- guilds_ag [guilds_ag $Guild == 'Undefined Saprotroph',]
soil <- guilds_ag [guilds_ag $Guild == 'Soil Saprotroph',]
wood <- guilds_ag [guilds_ag $Guild == 'Wood Saprotroph',]
ppath <- guilds_ag [guilds_ag $Guild == 'Plant Pathogen',]

guild_ag <- rbind(path,ecto,eric,fung,plant,endo,und,soil,wood,ppath)

guild_ag <- do.call(data.frame, guild_ag)

guild_ag$Guild <- as.character(guild_ag$Guild)

guild_ag[guild_ag=="Ectomycorrhizal"] <- "Ectomycorrhizal???"
guild_ag[guild_ag=="Animal Pathogen"] <- "Animal Pathogen*"
guild_ag[guild_ag=="Undefined Saprotroph"] <- "Undefined Saprotroph*"
guild_ag[guild_ag=="Wood Saprotroph"] <- "Wood Saprotroph*"
guild_ag[guild_ag=="Ericoid Mycorrhizal"] <- "Ericoid Mycorrhizal???"
guild_ag[guild_ag=="Fungal Parasite"] <- "Fungal Parasite*???"
guild_ag[guild_ag=="Plant Saprotroph"] <- "Plant Saprotroph*???"
guild_ag[guild_ag=="Plant Pathogen"] <- "Plant Pathogen*???"
guild_ag[guild_ag=="Soil Saprotroph"] <- "Soil Saprotroph*"

guild_ag <- guild_ag[guild_ag$Drought == 'Drought',]    

guild_ag <- guild_ag[guild_ag$LandUse == 'Reference',]   

guild_ag$Guild <- as.factor(guild_ag$Guild)

guild_ag$Day <- as.factor(guild_ag$Day)

library("RColorBrewer")

brewer.pal(n=9,"Set1")

set2 <- c("#E41A1C", "#377EB8", "#4DAF4A" ,"#984EA3" ,"#FF7F00" ,"#FFFF33" ,"#A65628", "#F781BF","#999999", "black", "powderpuff")

guild_ag1 <- guild_ag

guilds_legend <- 'Fungal Guilds'

guild1 <- ggplot(guild_ag1, aes(x=guild_ag1$Day, y=guild_ag1$Abundance.mean, group=guild_ag1$Guild)) + 
  geom_point(position=position_dodge(width=0.1),aes(color=guild_ag1$Guild), size=2) +
  geom_line(position=position_dodge(width=0.1),aes(color=guild_ag1$Guild),size=1) +
  #geom_errorbar(position=position_dodge(width=0.9),aes(ymin=resp2.2$Cmin.mean-resp2.2$se, ymax=resp2.2$Cmin.mean+resp2.2$se), width=3, size=.75) +
  labs(list(x ="")) +
  theme_classic() +
  scale_color_manual(guilds_legend,values=c("#E41A1C", "#377EB8", "#4DAF4A" ,"#984EA3" ,"#FF7F00" ,"#FFFF33" ,"#A65628", "#F781BF","#999999", "black")) +
  theme(axis.title=element_text(size=15)) +
  theme(text=element_text(size=15)) +
  theme(legend.text = element_text(color = "black", size = 12)) +
  xlab("Day") +
...
guild_ag <- guild_ag[guild_ag$LandUse == 'Disturbed',]   
...
guild2 <- ggplot(guild_ag2, aes(x=guild_ag2$Day, y=guild_ag2$Abundance.mean, group=guild_ag2$Guild)) + 
  geom_point(position=position_dodge(width=0.1),aes(color=guild_ag2$Guild), size=2) +
  geom_line(position=position_dodge(width=0.1),aes(color=guild_ag2$Guild),size=1) +
  #geom_errorbar(position=position_dodge(width=0.9),aes(ymin=resp2.2$Cmin.mean-resp2.2$se, ymax=resp2.2$Cmin.mean+resp2.2$se), width=3, size=.75) +
  labs(list(x ="")) +
  theme_classic() +
  scale_color_manual(values=c("#E41A1C", "#377EB8", "#4DAF4A" ,"#984EA3" ,"#FF7F00" ,"#FFFF33" ,"#A65628", "#F781BF","#999999", "black")) +
  theme(axis.title=element_text(size=15)) +
...
guild_ag <- guild_ag[guild_ag$Drought == 'Control',]    
...
guild3 <- ggplot(guild_ag3, aes(x=guild_ag3$Day, y=guild_ag3$Abundance.mean, group=guild_ag3$Guild)) + 
  geom_point(position=position_dodge(width=0.1),aes(color=guild_ag3$Guild), size=2) +
  geom_line(position=position_dodge(width=0.1),aes(color=guild_ag3$Guild),size=1) +
  #geom_errorbar(position=position_dodge(width=0.9),aes(ymin=resp2.2$Cmin.mean-resp2.2$se, ymax=resp2.2$Cmin.mean+resp2.2$se), width=3, size=.75) +
  labs(list(x ="")) +
  theme_classic() +
  scale_color_manual(guilds_legend, values=c("#E41A1C", "#377EB8", "#4DAF4A" ,"#984EA3" ,"#FF7F00" ,"#FFFF33" ,"#A65628", "#F781BF","#999999", "black")) +
...
guild4 <- ggplot(guild_ag4, aes(x=guild_ag4$Day, y=guild_ag4$Abundance.mean, group=guild_ag4$Guild)) + 
  geom_point(position=position_dodge(width=0.1),aes(color=guild_ag4$Guild), size=2) +
  geom_line(position=position_dodge(width=0.1),aes(color=guild_ag4$Guild),size=1) +
  #geom_errorbar(position=position_dodge(width=0.9),aes(ymin=resp2.2$Cmin.mean-resp2.2$se, ymax=resp2.2$Cmin.mean+resp2.2$se), width=3, size=.75) +
  labs(list(x ="")) +
  theme_classic() +
  scale_color_manual(values=c("#E41A1C", "#377EB8", "#4DAF4A" ,"#984EA3" ,"#FF7F00" ,"#FFFF33" ,"#A65628", "#F781BF","#999999", "black")) +
...
prow <- plot_grid(
  ...
  guild1 + theme(legend.position="none"),
  guild2 + theme(legend.position="none"),
  ncol = 2,
  align = 'hv',
  labels = c("A             Reference - Drought", "B             Disturbed - Drought", "C             Reference - Drought", "D             Disturbed - Drought", "E             Reference - Drought", "F             Disturbed - Drought"),
  label_size=16,
  hjust=0,
...
prow <- plot_grid(
  ...
  guild3 + theme(legend.position="none"),
  guild4 + theme(legend.position="none"),
  ncol = 2,
  align = 'hv',
  labels = c("A             Reference - Control", "B             Disturbed - Control", "C             Reference - Control", "D             Disturbed - Control", "E             Reference - Control", "F             Disturbed - Control"),
  label_size=16,
  hjust=0,
  label_y=1.025, scale = 0.9
)
...
legend3 <- get_legend(guild1 + theme(legend.box.margin = margin(0, 0, 0, 2)))


jpeg(filename="legends1.jpeg", bg="transparent", res=600, units = "in", height=10, width=4)


leg <- plot_grid(legend1, legend2,legend3,
...
legend6 <- get_legend(guild3 + theme(legend.box.margin = margin(0, 0, 0, 2)))


jpeg(filename="legends2.jpeg", bg="transparent", res=600, units = "in", height=10, width=4)


leg <- plot_grid(legend4, legend5,legend6,
